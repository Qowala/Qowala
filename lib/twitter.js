		...
		tagsModel.getUserTags(userId, function(tags){
			...
			T.get('lists/list', {}, function (err, data, response) {
			  ...
			  for(var i = 0; i < data.length; i++){
			  	usersSite[i] = data[i].id;
			  };

			  	setInterval(function(){
					T.get('lists/statuses', {list_id: data[1].id, count: 50}, function(err, allUsers, response){
				

						console.log('allUsers: ', allUsers);
						for(var i = allUsers.length-1; i >= 0; i--){
							calculateFrequency(allUsers[i], function(updatedTags, tagsStats){
								users.broadcast(userId, 'tweet', {tweet:allUsers[i], streamSource:'user', updatedTags:updatedTags, tagsStats:tagsStats});
							});
						};
					});
			  	}, 60000);
