...
var twitterAgent = {
	...
	initListsStream: function(userId, token, tokenSecret, lists){
		...
		var time = 61000;
		console.log('Gonna stream : ', lists);
		// console.log('Beginning the API Lists stream with lists: ', lists);
		

		var allTweetsFromAllLists = {};

		function getAllTweetsFromAllLists(i, allListsIndexes) {
			if(lists){
				if( i < lists.length ) {
					if (allListsIndexes[lists[i].id]){
						var since_id = allListsIndexes[lists[i].id];
					}
					else{
						var since_id = "623205711426781184";
					}
					console.log(lists[i].id, ' : since_id: ', since_id);
					...
					function(err, listTweets, response){
						allTweetsFromAllLists[lists[i].id] = listTweets;
						if(listTweets != undefined){
							console.log('listTweets are not undefined')
							if(listTweets[0] != undefined){
								console.log('listTweets[0] are not undefined')
								allListsIndexes[lists[i].id] = listTweets[0].id_str;
								console.log(lists[i].id, ': ', allListsIndexes[lists[i].id])
							}
						}
						if(response){
							if(response.headers){
						...
						if( err ) {
							...
							getAllTweetsFromAllLists(i+1, allListsIndexes);
						}
					});	
				}
				else{
					users.setListsTweetsCache(userId, allTweetsFromAllLists);
					console.log('allListsIndexes: ', allListsIndexes);
					users.setListsIndex(userId, allListsIndexes);
					// console.log('There are', allTweetsFromAllLists.length, ' elements in allTweetsFromAllLists');
					// console.log('allTweetsFromAllLists : ', allTweetsFromAllLists);
		...
		function repeatGetAllTweetsFromAllLists(){
			...
			users.getListsIndex(userId, function(allListsIndexes){
				console.log('In the all list index: ', allListsIndexes);
