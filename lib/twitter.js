...
var twitterAgent = {
   ...
   * Follow someone
   */
  followUser: function(userId, otherUserId, token, tokenSecret) {
    var T = new Twit({
      consumer_key: configTwitter.consumerKey,
      consumer_secret: configTwitter.consumerSecret,
      access_token: token,
    ...
    T.post('friendships/create',{user_id:otherUserId}, function (err, data, response) {
      if(err){
        console.error('Error: ', err);
      };
    });
   ...
   * Unfollow someone
   */
  unfollowUser: function(userId, otherUserId, token, tokenSecret) {
    var T = new Twit({
      consumer_key: configTwitter.consumerKey,
      consumer_secret: configTwitter.consumerSecret,
      access_token: token,
    ...
    T.post('friendships/destroy',{user_id:otherUserId}, function (err, data, response) {
      if(err){
        console.error('Error: ', err);
      };
    });
   ...
   * Get
   */
  getFollowedBy: function(userId, otherUserId, token, tokenSecret, cb) {
    var T = new Twit({
      consumer_key: configTwitter.consumerKey,
      consumer_secret: configTwitter.consumerSecret,
      access_token: token,
    ...
    T.get('followers/list',{user_id:otherUserId,count:50}, function (errOther, dataOther, responseOther) {
      if(errOther){
        console.error('Error: ', errOther);
      };
      T.get('followers/list',{user_id:userId,count:50}, function (errLocal, dataLocal, responseLocal) {
        if(errLocal){
          console.error('Error: ', errLocal);
        };

        var followers = {};
        if (dataLocal && dataOther) {
          for (var i = 0; i < dataLocal.users.length; i++) {
            if (Object.keys(followers).length > 2) break;
            for (var j = 0; j < dataOther.users.length; j++) {
              if (Object.keys(followers).length > 2) break;
              if (dataLocal.users[i].id == dataOther.users[j].id) {
                followers[dataLocal.users[i].screen_name] = {
                  name: dataLocal.users[i].name,
                  link: 'http://twitter.com/'+dataLocal.users[i].screen_name
                }
              }
            };
          };
          ...
          while(Object.keys(followers).length < 3) {
            followers[dataOther.users[i].screen_name] = {
              name: dataOther.users[i].name,
              link: 'http://twitter.com/'+dataOther.users[i].screen_name
            }
            i++;
          }
        }
