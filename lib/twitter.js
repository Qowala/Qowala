...
var twitterAgent = {
	...
	initStream: function(userId, token, tokenSecret){
		...
		tagsModel.getUserTags(userId, function(tags){
			...
			if(streamSearch[userId]){
				streamSearch[userId].stop();
			}
			...
			if(!tagsToTrack[0]){
				...
				streamSearch[userId] = T.stream('statuses/filter', { track: tagsToTrack });

				streamSearch[userId].on('tweet', function (tweet) {
				...
				streamSearch[userId].on('disconnect', function (disconnectMessage) {
					console.log('Log: Connection got closed by Twitter');
				});
			}
		});
	...
	stopStream: function(userId){
		...
		if(streamSearch[userId]){
			console.log('Stopping search stream');
			streamSearch[userId].stop();
		}
