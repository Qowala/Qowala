...
module.exports = function(io) {
  ...
  io.sockets.on('connection', function(socket){
    ...
    socket.on('followUser', function(otherUserId){
      users.followUser(localUserId, otherUserId);
    });

    socket.on('unfollowUser', function(otherUserId){
      users.unfollowUser(localUserId, otherUserId);
    });

    socket.on('getFollowedBy', function(otherUserId){
      users.getFollowedBy(localUserId, otherUserId, function(followers){
        socket.emit('returnFollowedBy', followers);
      });
    });
    ...
    socket.on('retweet', function(tweetId){
      ...
      users.sendRetweet(localUserId, tweetId, function(code, data){
        if(code === 200){
          // socket.emit('retweetedId', retweetedId);
          console.log('Log: Retweet sent with id ', data.id_str, '!');
        }
        else {
          console.log('Log: an error occured while retweeting');
        }
      });
    });
