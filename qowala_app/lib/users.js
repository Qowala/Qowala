...
var users = {
    ...
    /**
     * Push cache of user's friends
     * @param {Number}    userId   Users ID
     * @param {Object}    data     Data
     */
    pushUserFriendsCache: function(userId, data){
        usersModel.pushUserFriendsCache(userId, data);
    },

    /**
     * Get cache of user's friends
     * @param {Number}    userId   Users ID
     * @param {Function}  cb       Callback
     */
    getUserFriendsCache: function(userId, cb){
        usersModel.getUserFriendsCache(userId, cb);
    },
     ...
     * @param {Function}  cb       Callback
     */
    getUserFriends: function(userId, cb){
        users.getUserTokens(userId, function(token, tokenSecret){
            twitterAgent.getUserFriends(userId, token, tokenSecret, cb);
        });
    },

    /**
     * Push user's notifications
     * @param {Number} userId           User's ID
     * @param {Array}  notifications    User's notifications
     */
    pushUserNotifications: function(userId, notifications){
        usersModel.pushNotifications(userId, notifications);
    },
     ...
     * @param {Array}  notifications    User's notifications
     */
    setUserNotifications: function(userId, notifications){
        usersModel.updateNotifications(userId, notifications);
    },
     ...
     * @param {Function}  cb       Callback
     */
    getUserNotifications: function(userId, cb){
        usersModel.getNotifications(userId, cb);
    },
    ...
    /**
     * Search a user according to keyword
     * @param  {Number}   userId  User ID
     * @param  {String}   tweetID Tweet ID to be shown
     * @param  {Function} cb      Callback
     */
    searchUser: function(userId, keyword, cb){
      // Clean the keyword from special characters
      keyword = keyword.replace(/[*/\^.${}()[\]]/g, '');
      users.getUserFriendsCache(userId, function(friends){
        function matchFriends(friends, keyword){
          var friendsList = friends.users;
          var matchedFriends = [];
          for (var i = 0; i < friendsList.length; i++) {
            if(friendsList[i].screen_name.search(new RegExp(keyword, "i")) !== -1 || friendsList[i].name.search(new RegExp(keyword, "i")) !== -1){
              matchedFriends.push(friendsList[i]);
            }
          }
        ...
        if(friends === undefined){
          users.getUserFriends(userId, function(friends){
            var matchedFriends = matchFriends(friends, keyword);
            cb(matchedFriends);
          });
        } else {
          var matchedFriends = matchFriends(friends, keyword);
    ...
    followUser: function(userId, otherUserId) {
        users.getUserTokens(userId, function(token, tokenSecret){
            twitterAgent.followUser(userId, otherUserId, token, tokenSecret);
        });
    },

    unfollowUser: function(userId, otherUserId) {
        users.getUserTokens(userId, function(token, tokenSecret){
            twitterAgent.unfollowUser(userId, otherUserId, token, tokenSecret);
        });
    },

    getFollowedBy: function(userId, otherUserId, cb) {
        users.getUserTokens(userId, function(token, tokenSecret){
            twitterAgent.getFollowedBy(userId, otherUserId, token, tokenSecret, cb);
        });
    }
